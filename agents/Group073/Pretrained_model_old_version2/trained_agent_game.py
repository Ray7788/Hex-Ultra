...
import trained_agent_utils as utils
...
class MultiHexGame():
    ...
    '''
    def __init__(self, boards, models, temperature, temperature_decay):
    ...
    def batched_single_move(self, model):        
        ...
        for idx in range(len(self.current_boards)):
            correct_position = utils.correct_position1d(positions1d[idx].item(), self.board_size, self.boards[self.current_boards[idx]].player)
