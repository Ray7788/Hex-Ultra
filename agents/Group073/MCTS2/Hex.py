...
class HexBoard:
    ...
    def __init__(self, row=cfg.BOARD_ROW, col=cfg.BOARD_COL):
        self.row = row
        self.col = col
    ...
    def drop_stone(self, x, y, colour):
        ...
        assert self.board_graph[y][x].occupied is False and self.available_positions[
            y, x], f"({x}, {y}) has been occupied!"

        # update the board
        self.board_graph[y][x].occupy(colour)
        self.available_positions[y, x] = False
        # self.updateNumpyBoard(x, y, colour)
