...
class AlphaZeroTestSimulator:
    ...
    def simulate(self):
        ...
        for step in range(cfg.BOARD_ROW * cfg.BOARD_COL):
            # obtain decision of the current player
            x, y, distribution = self.current_player.make_decision(step, iterations=800)
            # save the state of the step
            self.state_queue.append(GameState(board=self.board, initial_colour=self.current_player.returnColour(),
                                              current_player=self.current_player, distribution=distribution))
            if x is None and y is None:
...
class MixSimulator:
    def __init__(self, net1, net2=None, visualization=True, device="cuda:0"):
        # initialize model
        self.net1 = net1
        self.net2 = net2
        # initialize device
...
if __name__ == "__main__":
    ...
    net1 = HexNetwork()
    net1.load_state_dict(torch.load("./hex11-20180712-3362.policy.pth")['policy']['net'])

    # net1.load_state_dict(torch.load("./module/net19000.pkl"))
    # net1.cuda()

    net2 = HexNetwork()
    net2.load_state_dict(torch.load("./hex11-20180712-3362.policy.pth")['policy']['net'])
    ...
    for _ in range(1):
        # game = HexSimulator()
        # game = AlphaZeroTestSimulator(net1, net2, visualization=True, device="cpu")
        game = MixSimulator(net1, visualization=True, device="cpu")
        game.simulate()
